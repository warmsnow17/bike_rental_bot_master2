...
# @dp.message_handler(state=states.RentRequestState.location, content_types=types.ContentType.TEXT)
# async def rent_request_delivery_location_text_received(message: types.Message, user: User, replies: dict[str, str], state: FSMContext, options: dict[str, str]):
#
#     # Проверяем, является ли текст ссылкой на геолокацию
#     location_url_pattern = re.compile(r'https?://maps\.app\.goo\.gl/.*', re.IGNORECASE)
#     is_location_url = location_url_pattern.match(message.text)
#
#     if is_location_url:
#         await message.answer("Пожалуйста, используйте встроенную функцию Telegram для отправки геолокации, а не ссылку.")
#     else:
...
#
#         await message.answer("Пожалуйста, отправьте вашу геолокацию через встроенную функцию Telegram:", reply_markup=location_keyboard)
#
#     await states.RentRequestState.location.set()
...
#
# @dp.message_handler(state=states.RentRequestState.location, content_types=types.ContentType.TEXT)
# async def rent_request_delivery_location_text(message: types.Message, user: User, replies: dict[str, str], state: FSMContext, options: dict[str, str]):
#     coordinates = extract_coordinates_from_google_maps_url(message.text)
#
#     if coordinates:
#         lat, lon = coordinates
...
#                 color_label=helpers.language.get_translation(user.language, '{}_color_label'.format(data.get('color')),
#                                                              data.get('color')),
#                 keyless_label=helpers.language.get_translation(user.language,
#                                                                'request_{}_answer_label'.format(data.get('keyless')),
#                                                                data.get('keyless')),
#                 abs_label=helpers.language.get_translation(user.language, 'request_{}_answer_label'.format(data.get('abs')),
#                                                            data.get('abs')),
#                 helmets_label=helpers.language.get_translation(user.language,
#                                                                '{}_helmets_label'.format(data.get('helmets', 'no')),
#                                                                data.get('helmets', 'no')),
#                 **data
#             )
#         else:
...
#                                                                '{}_helmets_label'.format(data.get('helmets', 'no')),
#                                                                data.get('helmets', 'no')),
#                 rent_type_label=helpers.language.get_translation(user.language,
#                                                                  '{}_rent_label'.format(data.get('rent_type')),
#                                                                  data.get('rent_type')),
#                 rent_type_amount_label=helpers.language.get_translation(user.language, '{}_rent_type_amount_label'.format(
#                     data.get('rent_type')), data.get('rent_type')),
#                 **data
#             )
#         await message.answer(reply_text, reply_markup=keyboards.ConfirmationKeyboard(language=user.language).markup())
#     else:
#         reply_text = replies.get('invalid_google_maps_url', 'Некорректная ссылка на Google Maps. Пожалуйста, отправьте правильную ссылку или локацию.')
#         await message.answer(reply_text, reply_markup=None)
