...
class RentalRequestResource(Model):
    page_size: int = 100  # измените значение с 10 на 100

    async def get_queryset(self, request: Request, *args, **kwargs):
        queryset = self.model.all()

        # Добавьте сортировку по убыванию здесь
        queryset = queryset.order_by('-created_at')

        # Примените фильтры
        for filter_obj in self.filters:
            if isinstance(filter_obj, str):
                filter_obj = self.app.filters[filter_obj]
            queryset = await filter_obj.apply(request, queryset, self)
